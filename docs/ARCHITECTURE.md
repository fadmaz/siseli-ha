...
- **Publish only what this payload evidences; one writer per key.** Every branch of `_try_ascii_schema` is `if len(vals) >= n` (`parsers.py:1457`, `:1358`, `:1454`); no defaults, no constants, no cross-block aliases. The only cross-payload reads are the `bat_v` cache fallback (`:827-829`) and previous energy totals (`:773`). Enforced by `tests/test_truthfulness.py:35-94` (no fabrication), `:97-179` (single writer), `:554-574` (golden parity).

...
| `parsers.py` | 2426 | TCP reassembly (`:65`, `:565`), MQTT framing (`:209`, `:423`, `:468`), `SolarParser` (`:799`) with `parse_payload` (`:2213`) and the positional decoder `_try_ascii_schema` (`:1600`), energy integrators (`:1011`), heartbeat and deferred-publish flush (`:700`, `:715`, `:730`), energy-clock persistence (`:647`, `:655`) |
...
| `state.py` | 149 | `LAST_STATE` (`:12`), `STATE_LOCK` (`:21`), `PUBLISH_LOCK` (`:149`), every cross-thread flag (`:26`, `:35`, `:42`, `:52`, `:62`), cadence measurement (`:65-82`), `atomic_write_json` (`:98`) |
...
Every thread that publishes the state topics holds `PUBLISH_LOCK` (`state.py:149`) from its snapshot to the throttle bookkeeping: the capture thread in `parse_payload` (`parsers.py:2213`), the health thread in `republish_state` (`parsers.py:730`) and the paho thread in `on_connect` (`mqtt.py:323`). Without it a snapshot taken first could be published last and stay retained, stepping an energy counter back. It is taken before `STATE_LOCK`, never inside it.
...
Status: **FIXED in 2.6.24**, but the end-to-end effect on a real dual-homed host is still unverified. Every frame now carries `Ether.src` (and the poisoning replies ARP `hwsrc`) from `resolve_own_mac`, which treats scapy's all-zero answer as unresolved; unresolved sends byte-identical frames to before. `restore_arp` uses the cached MAC because it runs in a signal handler. `ArpSpoofer.report_source_mac` logs the MAC used next to `route_mac_for`'s answer and warns when they differ — that line is the on-host evidence.
...
- **Publish only what this payload evidences; one writer per key.** Every branch of `_try_ascii_schema` is `if len(vals) >= n` (`parsers.py:1600`, `:1358`, `:1454`); no defaults, no constants, no cross-block aliases. The only cross-payload reads are the `bat_v` cache fallback (`:827-829`) and previous energy totals (`:773`). Enforced by `tests/test_truthfulness.py:35-94` (no fabrication), `:97-179` (single writer), `:554-574` (golden parity).
- **Timing bounds come from measured cadence, not from another option.** `state.observed_telemetry_interval` (`state.py:55-62`) feeds both `effective_telemetry_timeout` (`core.py:499-526`) and `_energy_max_dt` (`parsers.py:882`); `UPDATE_INTERVAL_SEC` is a publish throttle and says nothing about the inverter (`config.py:68`). The measured cadence is a fixture (`tests/test_core.py:603-692`, `tests/test_truthfulness.py:394-470`).

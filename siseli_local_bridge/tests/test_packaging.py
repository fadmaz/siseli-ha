...
ADDON = ROOT / "siseli_local_bridge"
CONFIG_PY = ADDON / "src" / "siseli_local_bridge" / "config.py"
...
class TestVersionConsistency(unittest.TestCase):
    ...
    def setUp(self):
        from src.siseli_local_bridge.version import __version__
    ...
    def test_the_root_changelog_points_at_the_canonical_one(self):
        ...
        self.assertIn("siseli_local_bridge/CHANGELOG.md", text)
    ...
    def test_run_sh_carries_no_version_literal(self):
        ...
        self.assertNotRegex(
            run_sh,
            r"Siseli Local Bridge\s+[0-9]",
            "run.sh must not print a version; core.log_startup_configuration() does",
        )

    def test_core_reports_the_single_source(self):
        from src.siseli_local_bridge import version as version_mod

        core_src = (ADDON / "src" / "siseli_local_bridge" / "core.py").read_text(encoding="utf-8")
        self.assertNotRegex(
            core_src,
            r'^VERSION\s*=\s*["\']',
...
class TestPackagingMetadata(unittest.TestCase):
    ...
    def test_packages_are_declared_explicitly(self):
        """siseli_local_bridge/ has no __init__.py, so auto-discovery finds nothing and
        would build an empty wheel."""
        text = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
        self.assertIn("[tool.setuptools]", text)
        self.assertIn('packages = ["src", "src.siseli_local_bridge"]', text)
...
class TestDocumentationSplit(unittest.TestCase):
    ...
    Documentation tab -- which renders siseli_local_bridge/DOCS.md -- was empty, so users were
    ...
    def test_the_readme_points_at_the_tab(self):
        self.assertIn("siseli_local_bridge/DOCS.md", self.readme)
    ...
    def test_the_docs_tab_uses_absolute_links(self):
        ...
        wrong on GitHub, where they would resolve against siseli_local_bridge/ rather than the
        repository root."""
...
class TestArchitectureDocAnchors(unittest.TestCase):
    ...
    BARE = {
        "core.py": "siseli_local_bridge/src/siseli_local_bridge/core.py",
        "parsers.py": "siseli_local_bridge/src/siseli_local_bridge/parsers.py",
        "mqtt.py": "siseli_local_bridge/src/siseli_local_bridge/mqtt.py",
        "sensors.py": "siseli_local_bridge/src/siseli_local_bridge/sensors.py",
        "state.py": "siseli_local_bridge/src/siseli_local_bridge/state.py",
        "config.py": "siseli_local_bridge/src/siseli_local_bridge/config.py",
        "loggers.py": "siseli_local_bridge/src/siseli_local_bridge/loggers.py",
        "version.py": "siseli_local_bridge/src/siseli_local_bridge/version.py",
        "DOCS.md": "siseli_local_bridge/DOCS.md",
        "config.yaml": "siseli_local_bridge/config.yaml",
        "run.sh": "siseli_local_bridge/run.sh",
        "requirements.txt": "siseli_local_bridge/requirements.txt",
        "translations/en.yaml": "siseli_local_bridge/translations/en.yaml",
        "CHANGELOG.md": "siseli_local_bridge/CHANGELOG.md",
    }
...
class TestDiagnosticVocabularyIsDocumented(unittest.TestCase):
    ...
    def test_every_body_shape_appears_in_the_docs(self):
        ...
        source = (ADDON / "src" / "siseli_local_bridge" / "parsers.py").read_text(encoding="utf-8")
...
class TestDocumentedCountsAreCurrent(unittest.TestCase):
    ...
    def test_the_undecoded_count_is_current(self):
        ...
        from src.siseli_local_bridge.sensors import SENSORS, UNDECODED_SENSOR_KEYS
...
class TestDockerContext(unittest.TestCase):
    ...
    src/__pycache__, src/siseli_local_bridge/__pycache__, .coverage and siseli_local_bridge.egg-info
...
class TestPinsAgree(unittest.TestCase):
    ...
    def test_the_runtime_pins_live_in_one_file(self):
        ...
        self.assertRegex(
            pyproject,
            r'dependencies\s*=\s*\{\s*file\s*=\s*\[\s*"siseli_local_bridge/requirements.txt"',
...
class TestDebugFlagWiring(unittest.TestCase):
    ...
    def test_every_declared_flag_is_offered_in_the_schema(self):
        from src.siseli_local_bridge.config import DEBUG_FLAG_NAMES
...
class TestShippedDefaultsSatisfyTheSchema(unittest.TestCase):
    ...
    def test_a_previously_valid_stored_configuration_still_installs(self):
        ...
        stored = {
            ...
            "DEVICE_ID": "siseli_local_inverter_1",
            ...
            "STATE_TOPIC": "siseli/siseli_local_inverter_1/state",
            "AVAILABILITY_TOPIC": "siseli/siseli_local_inverter_1/availability",
            "SNIFF_IFACE": "",
            "LOG_VERBOSE": True,
            "LOG_LEVEL": "info",
